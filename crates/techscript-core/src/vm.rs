...
impl VM {
    ...
    pub fn run_event_loop(&mut self) -> TechResult<()> {
        ...
        let op: OpCode = OpCode::try_from(byte).map_err(|_| {
            ...
            match op {
                ...
                OpCode::Import => {
                    ...
                    match name.as_str() {
                        "net"     => crate::stdlib::net::register_net_module(&mut self.globals),
                        "web"     => crate::stdlib::web::register_web_module(&mut self.globals),
                        "gui"     => crate::stdlib::web::register_gui_module(&mut self.globals),
                        "three_d" => crate::stdlib::web::register_three_d_module(&mut self.globals),
                        "anime"   => crate::stdlib::web::register_anime_module(&mut self.globals),
                        "debug"   => crate::stdlib::debug::register_debug_module(&mut self.globals),
                        "api"     => crate::stdlib::api::register_api_module(&mut self.globals),
                        // Built-in modules already registered at startup
                        "math" | "fs" | "os" | "random" | "json" | "crypto" | "date" => {}
                        _ => return Err(self.runtime_error(format!("Unknown module: '{}'. Available: net, web, gui, three_d, anime, api, debug, math, fs, os, random, json, crypto, date", name))),
                    }
                }
    ...
    fn string_property(&self, s: &str, name: &str) -> TechResult<Value> {
        match name {
            ...
            // Case/whitespace
            "upper" => Ok(self.make_native_str(s, "upper", |s, _| Ok(Value::String(Rc::new(s.to_uppercase()))))),
            "lower" => Ok(self.make_native_str(s, "lower", |s, _| Ok(Value::String(Rc::new(s.to_lowercase()))))),
            "trim" => Ok(self.make_native_str(s, "trim", |s, _| Ok(Value::String(Rc::new(s.trim().to_string()))))),

            // Search/replace
            "contains" => Ok(self.make_native_str(s, "contains", |s, args| {
                let sub = args.first().map(|v| v.display_string()).unwrap_or_default();
                Ok(Value::Bool(s.contains(sub.as_str())))
            })),
            "starts_with" => Ok(self.make_native_str(s, "starts_with", |s, args| {
                let p = args.first().map(|v| v.display_string()).unwrap_or_default();
                Ok(Value::Bool(s.starts_with(p.as_str())))
            })),
            "ends_with" => Ok(self.make_native_str(s, "ends_with", |s, args| {
                let p = args.first().map(|v| v.display_string()).unwrap_or_default();
                Ok(Value::Bool(s.ends_with(p.as_str())))
            })),
            "find" => Ok(self.make_native_str(s, "find", |s, args| {
                let sub = args.first().map(|v| v.display_string()).unwrap_or_default();
                Ok(Value::Int(s.find(sub.as_str()).map(|i| i as i64).unwrap_or(-1)))
            })),
            "replace" => Ok(self.make_native_str(s, "replace", |s, args| {
                let from = args.first().map(|v| v.display_string()).unwrap_or_default();
                let to = args.get(1).map(|v| v.display_string()).unwrap_or_default();
                Ok(Value::String(Rc::new(s.replacen(from.as_str(), to.as_str(), 1))))
            })),
            "replace_all" => Ok(self.make_native_str(s, "replace_all", |s, args| {
                let from = args.first().map(|v| v.display_string()).unwrap_or_default();
                let to = args.get(1).map(|v| v.display_string()).unwrap_or_default();
                Ok(Value::String(Rc::new(s.replace(from.as_str(), to.as_str()))))
            })),
            "split" => Ok(self.make_native_str(s, "split", |s, args| {
                let sep = args.first().map(|v| v.display_string()).unwrap_or(" ".into());
    ...
    fn make_native_str(
        ...
        let captured = s.to_string();
        let method_name = method_name.to_string();
        Value::NativeFunction(Rc::new(NativeFnObj {
            name: format!("string.{}", method_name),
            func: Box::new(move |args: &[Value]| f(&captured, args)),
        }))
    }
    ...
    fn list_method_call(list: &Rc<RefCell<Vec<Value>>>, name: &str, args: &[Value]) -> Result<Value, String> {
        ...
        match name {
            ...
            "sort" => {
                l.sort_by(|a, b| a.compare(b).unwrap_or(std::cmp::Ordering::Equal));
                Ok(Value::None)
            }
            "reverse" => {
                l.reverse();
    ...
    fn op_index(&self, obj: &Value, idx: &Value) -> TechResult<Value> {
        match (obj, idx) {
            ...
            (Value::Map(m), Value::String(k)) => {
                let b = m.borrow();
                let alt = format!("_{}", k.as_ref());
                let direct = b.get(k.as_ref());
                let underscored = b.get(&alt);

                match (direct, underscored) {
                    // Prefer the underscored storage when the direct key is a callable (method) and the underscored key exists.
                    (Some(Value::NativeFunction(_)), Some(v2)) => Ok(v2.clone()),
                    (Some(Value::Closure(_)), Some(v2)) => Ok(v2.clone()),
                    (Some(v), _) => Ok(v.clone()),
                    (None, Some(v2)) => Ok(v2.clone()),
    ...
    fn op_set_index(&self, obj: &Value, idx: &Value, val: Value) -> TechResult<()> {
        match (obj, idx) {
            ...
            (Value::Map(m), Value::String(k)) => {
                ...
                if b.contains_key(k.as_ref()) {
                    b.insert(k.as_ref().clone(), val);
                } else {
                    // Mirror the lookup behavior: if the public key doesn't exist but the underscored key does, write there.
                    let alt = format!("_{}", k.as_ref());
                    if b.contains_key(&alt) {
                        b.insert(alt, val);
                    } else {
                        b.insert(k.as_ref().clone(), val);
                    }

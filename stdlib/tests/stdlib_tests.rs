...
fn test_web_module() {
    ...
    // Test that `start` returns an error when port is in use
    let start_result = start.call(
        &mut ctx,
        vec![
    ...
    // Test that `serve` returns an error when port is in use
    let serve_result = serve.call(&mut ctx, vec![RuntimeValue::Int(port as i64)]);

# 🐉 TechScript — Cinema Edition (v1.0.4.5)

<p align="center">
  <img src="https://raw.githubusercontent.com/Tcode-Motion/techscript/main/assets/techscript_v2_logo.png" alt="TechScript Dragon Logo" width="220">
</p>

<p align="center">
  <strong>The world's easiest programming language. Now more powerful than ever.</strong>
</p>

<div align="center">

![version](https://img.shields.io/badge/version-v1.0.4.5-7c3aed?style=for-the-badge)
![runtime](https://img.shields.io/badge/runtime-Native_Rust_VM-f97316?style=for-the-badge&logo=rust&logoColor=white)
![platform](https://img.shields.io/badge/platform-Windows%20%7C%20macOS%20%7C%20Linux%20%7C%20Android-22c55e?style=for-the-badge)
![license](https://img.shields.io/badge/license-MIT-3b82f6?style=for-the-badge)

</div>

<div align="center">

![deps](https://img.shields.io/badge/dependencies-NONE-00ff88?style=for-the-badge)
![stdlib](https://img.shields.io/badge/stdlib-180%2B_functions-7c3aed?style=for-the-badge)
![vscode](https://img.shields.io/badge/VS_Code-Extension_v1.0.3-007ACC?style=for-the-badge&logo=visualstudiocode&logoColor=white)
[![Author](https://img.shields.io/badge/Made_by-Tcode--Motion-181717?style=for-the-badge&logo=github)](https://github.com/Tcode-Motion/techscript)
[![Stars](https://img.shields.io/github/stars/Tcode-Motion/techscript?style=for-the-badge&color=yellow&label=⭐)](https://github.com/Tcode-Motion/techscript/stargazers)

</div>

---

<div align="center">

```
╔═════════════════════════════════════════════════════════════════════════╗
║                                                                         ║
║   No semicolons. No brackets. No confusing symbols.                     ║
║   Universal Support — Windows, Linux, Mac, & Termux!                    ║
║   Blazing Fast Native Rust VM + Premium 3D Animation Studio! ⚡        ║
╚═════════════════════════════════════════════════════════════════════════╝
```

</div>

---

## 🤔 What is TechScript? (Explain Like I'm 10)

Imagine you want to tell a computer to do something. Normally, computers only understand confusing code like this:

```javascript
const x = document.getElementById('name');
if (x !== null && x.value.length > 0) { ... }
```

**TechScript makes that feel like writing a sentence:**

```
make name = ask "What is your name? "
say f"Hello, {name}!"
```

TechScript is:
- 🟢 **A programming language** — writes code that runs natively.
- 🌐 **A web builder** — build full interactive websites without HTML/CSS.
- 🦀 **Powered by Native Rust** — blazing speed on Windows (Native x64).
- 🐍 **Universal Edition** — Automatically runs on Linux, Mac, and Android via built-in fallback.
- 📦 **Zero Logic Bloat** — No semicolons. No brackets. Just pure logic.

---

## 🚀 Key Features

| Feature | Description | Support |
|:---|:---|---|
| **Cinema Studio** | **Advanced 3D Motion Design & Accretion Disk Simulation** | ✅ v7.0 |
| **Transpilation** | Auto-convert TechScript to clean Python for production | ✅ YES |
| **Web Module** | Build UIs with `page.h1()`, `page.style()`, and `page.run()` | ✅ YES |
| **180+ Functions** | Rich StdLib (Math, Crypto, OS, FS, JSON, Date, etc.) | ✅ YES |
| **Native Binary** | Runs without any dependencies on Windows | ✅ YES |
| **Termux Force** | Optimized for Android development with built-in examples | ✅ YES |

---

## 📦 Installation

### 🪟 Windows (Recommended)
1. Download the latest **[TechScript_v1.0.4.5_Setup.exe](../../releases/latest)**.
2. Run it. Open PowerShell. Type `tech version`. Done. 🐉

### 🐧 Linux (Ubuntu, Kali, Arch)
**Super-Fast One-Liner (Auto-detects Native Rust binary):**
```bash
curl -fsSL https://raw.githubusercontent.com/Tcode-Motion/techscript/main/scripts/install.sh | bash
```

### 📱 Android (Termux)
```bash
pkg install python -y && pip install techscript-lang
```
*(v1.0.4.5+ is fully optimized for Termux with automatic Python-engine fallback)*

---

## 📖 Learn the Language

- **Variables**: `make count = 10`
- **Constants**: `keep PI = 3.14`
- **Logic**: `when score > 90 { say "Perfect!" }`
- **Loops**: `each item in [1, 2, 3] { say item }`
- **Functions**: `build greet(name) { say f"Hi {name}" }`
- **Web**: `use web`, `make p = WebPage()`, `p.run()`

Check out the **[Full Reference Guide](docs/REFERENCE.md)** for more.

---

## 🎨 VS Code Extension
Included in the installer! Or install manually:
```bash
code --install-extension vscode-extension/techscript-1.0.3.vsix
```

---

<p align="center">
  <img src="https://raw.githubusercontent.com/Tcode-Motion/techscript/main/assets/techscript_v2_logo.png" alt="TechScript Dragon" width="80">
  <br>
  <strong>Made with 🐉 and 🦀 by Tcode-Motion</strong>
</p>

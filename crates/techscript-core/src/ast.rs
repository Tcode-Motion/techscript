...
pub enum Stmt {
    ...
    Say { values: Vec<Expr> },
    /// `make <name>: <type> = <expr>`
    Set { name: String, type_ann: Option<String>, value: Expr },
    /// `keep <name>: <type> = <expr>`
    Const { name: String, type_ann: Option<String>, value: Expr },
    /// `<target> = | += | -= | *= | /= <expr>`
    ...
    /// `build <name>(<params>) { ... }` or `async build ...`
    Fn { name: String, params: Vec<Param>, body: Vec<Stmt>, is_async: bool },
    /// `model <name>(<parent>) { ... }`
...
pub enum Expr {
    ...
    Await { expression: Box<Expr> },
    Spawn { expression: Box<Expr> },
}

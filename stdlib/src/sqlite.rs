...
impl StdlibRegistry {
    pub fn register_sqlite(&mut self) {
        ...
        exports.insert("execute".to_string(), Rc::new(StdFunction {
            name: "execute".to_string(),
            arity: 2,
            callback: |_ctx, args| {
                let ptr = args[0].try_into_int().map_err(|_| RuntimeError::new(RuntimeErrorKind::InvalidOperation("expected int handle".to_string()), None, None))?;
        ...
        exports.insert("query".to_string(), Rc::new(StdFunction {
            ...
            callback: |_ctx, args| {
                ...
                Ok(RuntimeValue::List { items: Rc::new(RefCell::new(rows)), is_const: false })
            },
        }));

        exports.insert("close".to_string(), Rc::new(StdFunction {
            name: "close".to_string(),
            arity: 1,
            callback: |_ctx, args| {
                let ptr = args[0].try_into_int().map_err(|_| RuntimeError::new(RuntimeErrorKind::InvalidOperation("expected int handle".to_string()), None, None))?;
                unsafe { drop(Box::from_raw(ptr as *mut rusqlite::Connection)); }
                Ok(RuntimeValue::Null)
            },
        }));

        self.register_module("std.sqlite", StdlibModule {

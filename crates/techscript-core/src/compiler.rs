...
impl Compiler {
    ...
    fn compile_stmt(&mut self, stmt: &Stmt, line: usize) -> TechResult<()> {
        match stmt {
            ...
            Stmt::Set { name, value, type_ann: _ } | Stmt::Const { name, value, type_ann: _ } => {
            ...
            Stmt::Assign { target, op, value } => {
                match target {
                    Expr::Identifier(name) => {
                        ...
                        // Assignment is a statement: discard the assigned value.
                        self.emit(OpCode::Pop, line);
                    }
                    Expr::Member { obj, member } => {
                        ...
                        // SetProperty leaves the value on the stack; discard it for statement form.
                        self.emit(OpCode::Pop, line);
                    }
            ...
            Stmt::Class { name, parent, body } => {
                ...
                for stmt in body {
                    if let Stmt::Fn { name: method_name, params, body: method_body, is_async: _ } = stmt {
    ...
    fn compile_expr(&mut self, expr: &Expr, line: usize) -> TechResult<()> {
        match expr {
            ...
            Expr::Await { expression } => {
                self.compile_expr(expression, line)?;
                self.emit(OpCode::Await, line);
            }
            Expr::Spawn { expression } => {
                self.compile_expr(expression, line)?;
                self.emit(OpCode::Spawn, line);
            }

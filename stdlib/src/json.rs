...
mod tests {
    ...
    #[test]
    fn test_parse_json_value_simple() {
        // Null
        assert_eq!(
        ...
        assert_eq!(
            parse_json_value(serde_json::Value::Bool(false)),
            RuntimeValue::Bool(false)
        );

        // Int
        assert_eq!(
            parse_json_value(serde_json::Value::Number(serde_json::Number::from(42))),
            RuntimeValue::Int(42)
        );
        assert_eq!(
            parse_json_value(serde_json::Value::Number(serde_json::Number::from(-10))),
            RuntimeValue::Int(-10)
        );

        // Float
        assert_eq!(
            parse_json_value(serde_json::Value::Number(
        ...
        // String
        assert_eq!(
            parse_json_value(serde_json::Value::String("hello".to_string())),
            RuntimeValue::Str("hello".to_string())
        );
    }

    #[test]
    fn test_parse_json_value_complex() {
        // Array
        ...
        let parsed_arr = parse_json_value(arr);
        match parsed_arr {
            RuntimeValue::List { items, is_const } => {
                assert!(!is_const);
                let borrowed = items.borrow();
                assert_eq!(borrowed.len(), 3);
                assert_eq!(borrowed[0], RuntimeValue::Int(1));
                assert_eq!(borrowed[1], RuntimeValue::Str("two".to_string()));
                assert_eq!(borrowed[2], RuntimeValue::Bool(false));
            }
        ...
        let parsed_obj = parse_json_value(obj);
        match parsed_obj {
            RuntimeValue::Map { entries, is_const } => {
                assert!(!is_const);
                let borrowed = entries.borrow();
                assert_eq!(borrowed.len(), 2);
                assert_eq!(borrowed.get("key1").unwrap(), &RuntimeValue::Int(100));
                assert_eq!(
                    borrowed.get("key2").unwrap(),
                    &RuntimeValue::Str("value2".to_string())
                );
            }
            _ => panic!("Expected RuntimeValue::Map"),
        }
    }
}

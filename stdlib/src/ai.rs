            ...
            Rc::new(StdFunction {
                ...
                callback: |ctx, args| {
                    ...
                    match provider.as_str() {
                        ...
                        "gemini" => generate_gemini(&prompt),
                        "local" => generate_local(&prompt),
...
fn generate_openai(prompt: &str) -> Result<RuntimeValue, RuntimeError> {
    ...
    let content = json["choices"][0]["message"]["content"]
        ...
        .ok_or_else(|| {
            RuntimeError::new(
                RuntimeErrorKind::InvalidOperation("OpenAI response content empty".to_string()),
                None,
                None,
            )
        })?;
...
fn generate_gemini(prompt: &str) -> Result<RuntimeValue, RuntimeError> {
    ...
    let content = json["candidates"][0]["content"]["parts"][0]["text"]
        ...
        .ok_or_else(|| {
            RuntimeError::new(
                RuntimeErrorKind::InvalidOperation("Gemini response content empty".to_string()),
                None,
                None,
            )
        })?;
...
fn generate_local(prompt: &str) -> Result<RuntimeValue, RuntimeError> {

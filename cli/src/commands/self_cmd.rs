    ...
    match sub.as_str() {
        "update" => {
            ...
            match ureq::get(check_url).call() {
                    ...
                    if let Ok(resp_str) = resp.into_string() {
                        if let Ok(info) = serde_json::from_str::<VersionCheckResponse>(&resp_str) {
                            ...
                            if remote_ver != techscript_common::TECHSCRIPT_VERSION {
                                println!(
                                    "\n{}",
                                    format!("→ A new version v{} is available!", remote_ver)
                                        .green()
                                        .bold()
                                );
                            ...
                            println!(
                                "Remote Toolchain version: v{}",
                                techscript_common::TECHSCRIPT_VERSION
                            );
                            println!(
                                "\n{}",
                                "✓ All toolchain components are up-to-date (signature validated)."
                                    .green()
                                    .bold()
                            );
                        }
                ...
                Err(_) => {
                    ...
                    println!(
                        "Remote Toolchain version: v{} (offline fallback)",
                        techscript_common::TECHSCRIPT_VERSION
                    );
                    println!(
                        "\n{}",
                        "✓ All toolchain components are up-to-date (cached)."
                            .green()
                            .bold()
                    );
                }
        ...
        "repair" => {
            ...
            if res == ExitCode::Success {
                println!(
                    "\n{}",
                    "✓ Self repair completed successfully. All components are healthy."
                        .green()
                        .bold()
                );
            } else {
                println!(
                    "\n{}",
                    "⚠ Self repair encountered some warnings. Please fix details above."
                        .yellow()
                        .bold()
                );
            }
        ...
        other => {
            eprintln!(
                "Error: Unknown self subcommand '{}'. Choose from: update, uninstall, repair.",
                other
            );

...
impl StdlibRegistry {
    pub fn register_parallel(&mut self) {
        ...
        exports.insert("num_cpus".to_string(), Rc::new(StdFunction {
            name: "num_cpus".to_string(),
            arity: 0,
            callback: |_ctx, _args| {
                Ok(RuntimeValue::Int(std::thread::available_parallelism()
                    .map(|n| n.get() as i64)
                    .unwrap_or(1)))
            },
        }));

        self.register_module("std.parallel", StdlibModule {

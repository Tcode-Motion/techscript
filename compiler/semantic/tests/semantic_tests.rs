...
fn test_dsl_semantic_unknown_property_warning() {
    ...
    assert!(
        res.is_ok(),
        "Unknown property should be warning only: {:?}",
        diags
    );
    let warnings: Vec<_> = diags
        .iter()
        .filter(|d| d.code == ErrorCode::E0401)
        .collect();
...
fn test_dsl_semantic_duplicate_property_error() {
    ...
    let errors: Vec<_> = diags
        .iter()
        .filter(|d| d.code == ErrorCode::E0400)
        .collect();
...
fn test_dsl_semantic_missing_required_property() {
    ...
    let errors: Vec<_> = diags
        .iter()
        .filter(|d| d.code == ErrorCode::E0402)
        .collect();
    assert_eq!(errors.len(), 1);
    assert!(
        errors[0]
            .message
            .contains("missing required property 'label'")
            || errors[0]
                .message
                .contains("Missing required property 'label'")
    );
...
fn test_dsl_semantic_invalid_nested_block_warning() {
    ...
    assert!(
        res.is_ok(),
        "Invalid nest should be warning only: {:?}",
        diags
    );
    let warnings: Vec<_> = diags
        .iter()
        .filter(|d| d.code == ErrorCode::E0403)
        .collect();
    assert_eq!(warnings.len(), 1);

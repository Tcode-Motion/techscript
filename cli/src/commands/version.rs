...
pub fn execute() -> ExitCode {
    ...
    println!(
        "Language Server (LSP):    techscript-lsp v{}",
        techscript_common::TECHSCRIPT_VERSION
    );
    println!(
        "Formatter (tsfmt):        tsfmt v{}",
        techscript_common::TECHSCRIPT_VERSION
    );
    println!(
        "Linter (tslint):          tslint v{}",
        techscript_common::TECHSCRIPT_VERSION
    );
    println!(
        "Package Manager (tspm):   tspm v{}",
        techscript_common::TECHSCRIPT_VERSION
    );
    ...
    println!(
        "Installation Path:        {}",
        std::env::current_exe()
            .map(|p| p.parent().unwrap_or(p.as_path()).display().to_string())
            .unwrap_or_default()
    );
    println!(
        "{}",
